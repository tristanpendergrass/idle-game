...
-}
standardTick : Snapshot.Tick StandardTickState
standardTick =
    let
        mapGameAndToasts : Duration -> StandardTickState -> StandardTickState
        mapGameAndToasts duration state =
            let
                tickResult =
                    Game.tick duration state.cache state.game

                newCache =
                    if tickResult.bustCache then
                        getCache tickResult.game

                    else
                        state.cache
            in
            { game = tickResult.game
            , toasts = state.toasts ++ tickResult.toasts
            , bustCache = tickResult.bustCache
            , cache = newCache
            }
    in
...
-}
performantTick : Duration -> Snapshot.Tick ( Game, Cache )
performantTick duration =
    Snapshot.createTick duration
        (\d ( oldGame, oldCache ) ->
            let
                { game, bustCache } =
                    Game.tick d oldCache oldGame
            ...
            case List.Extra.getAt index mainMenuFrontend.games of
                Just activeGameAndSnapshot ->
                    let
                        ...
                        gameWithCache : Snapshot ( Game, Cache )
                        gameWithCache =
                            game
                                |> Snapshot.map
                                    (\g ->
                        ...
                        newInGameFrontend =
                            ...
                            , isVisible = isVisible
                            , games = inGameGames
                            , gameState = FastForward { original = gameWithCache, current = gameWithCache, whenItStarted = now }
...
updateInGame msg inGameFrontend =
    ...
    case msg of
            ...
            case inGameFrontend.gameState of
                FastForward { original, current, whenItStarted } ->
                    ...
                    if Time.posixToMillis nextInterval < Time.posixToMillis now then
                        ...
                        let
                            newSnapshotWithCache : Snapshot ( Game, Cache )
                            newSnapshotWithCache =
                                Snapshot.tickUntil (performantTick (Duration.minutes 1)) now current

                            ( newGame, newGameplayCache ) =
                                Snapshot.getValue newSnapshotWithCache

                            newViewCache : Cache
                            newViewCache =
                                getViewCache newGame.activeTab newGameplayCache newGame
                            ...
                            originalSnapshot : Snapshot Game
                            originalSnapshot =
                                original
                                    |> Snapshot.map Tuple.first
                            ...
                            newModal =
                                if Config.flags.debugTimePasses then
                                    ...
                                    createTimePassesModal (Duration.milliseconds (toFloat (Time.posixToMillis now - Time.posixToMillis whenItStarted))) originalSnapshot newSnapshot
                        in
                        ( InGame
                            (inGameFrontend
                                |> setGameState (Playing { gameplayCache = newGameplayCache, viewCache = newViewCache })
                                |> setGame newSnapshot
                    ...
                    ( InGame
                        (inGameFrontend
                            |> setPreview
                                (let
                                    cached =
                                        getByActivity activity viewCache
                                 in
                                 Just (Preview ( activity, cached.effects, cached.duration ))
                                )
                            |> setActivityExpanded
                                (if clickingActiveActivity then
                                    True
        ...
        HandleStopClick kind ->
            case inGameFrontend.gameState of
                Playing { viewCache } ->
                    let
                        cached =
                            getByActivity kind viewCache
                    in
                    ( InGame
                        (inGameFrontend
                            |> setActivity Nothing
                            |> setPreview (Just (Preview ( kind, cached.effects, cached.duration )))
                        )
                    , Cmd.none
                    )
        ...
        HandleSpellAssignmentClick activity spellResource ->
            case inGameFrontend.gameState of
                Playing _ ->
                    let
                                ...
                                (\game ->
                                    let
                                        updatedSpellAssignments =
                                            setByActivity activity (Just spellResource) game.spellAssignments
                                    in
        ...
        HandleAnimationFrame now ->
            case inGameFrontend.gameState of
                ...
                Playing { gameplayCache, viewCache } ->
                    if not inGameFrontend.isVisible then
                        ...
                        let
                            oldSnapshot : Snapshot StandardTickState
                            oldSnapshot =
                                getGame inGameFrontend
                                    |> Snapshot.map
                                        (\g ->
                            ...
                            updatedSnapshot : Snapshot StandardTickState
                            updatedSnapshot =
                                oldSnapshot
                            ...
                            newGameState =
                                Playing { gameplayCache = newGameplayCache, viewCache = newViewCache }
                        in
                        ( InGame
                            (inGameFrontend
                ...
                case inGameFrontend.gameState of
                    ...
                    Playing _ ->
                        let
                            ...
                            oldSnapshotWithCache : Snapshot ( Game, Cache )
                            oldSnapshotWithCache =
                                oldSnapshot
                                    |> Snapshot.map
                                        (\g ->
                                            let
                        ...
                        ( InGame
                            (inGameFrontend
                                |> setIsVisible True
                                |> setGameState (FastForward { original = oldSnapshotWithCache, current = oldSnapshotWithCache, whenItStarted = now })
                            )
                        , Task.perform HandleFastForward Time.now
                        )
                            ...
                            case purchaseResult of
                                Ok res ->
                                    let
                                        ...
                                        newModel =
                                            { inGameFrontend | gameState = Playing { gameplayCache = newGameplayCache, viewCache = newViewCache } }
                                                |> setActiveModal Nothing
            ...
            case inGameFrontend.gameState of
                Playing _ ->
                    let
                        ...
                        currentWithCache : Snapshot ( Game, Cache )
                        currentWithCache =
                            current
                                |> Snapshot.map
                                    (\g ->
                        ...
                        fastForwardState =
                            { original = currentWithCache, current = currentWithCache, whenItStarted = now }
                    in
                    ( InGame
                        (inGameFrontend
                        ...
                        firstGame :: restGames ->
                            ...
                            let
                                ...
                                newInGameFrontend =
                                    ...
                                    , games = inGameGames
                                    , gameState = FastForward { original = gameWithCache, current = gameWithCache, whenItStarted = Time.millisToPosix 0 }
                                    ...
                                    renderActivity ( activity, timer ) =
                                        let
                                            cached =
                                                getByActivity activity viewCache
                                        in
                                        IdleGame.Views.DetailViewContent.renderContent (IdleGame.Views.DetailViewContent.DetailViewActivity ( ( activity, cached.effects, cached.duration ), timer )) extraBottomPadding game
                                ...
                                div [ class "flex h-full w-full relative overflow-hidden" ]
                                    ...
                                    , div [ class "bg-base-100 drawer lg:drawer-open" ]
                                        [ input
                                            ...
                                            []
                                        , IdleGame.Views.Content.renderContent frontend game viewCache game.activeTab
...
getCache game =
    ...
    Activity.createRecordByFn
        (\activity ->
            ...
            in
            { effects = moddedEffects, duration = duration }
        )


getViewCache : Tab -> Cache -> Game -> Cache
getViewCache currentTab gameplayCache game =
    case currentTab of
        Tab.SkillTab currentSkill ->
            Activity.mapActivityRecordWithKey
                (\activity cached ->
                    let
                        activityStats =
                            getActivityStats activity
                    in
                    if activityStats.skill == currentSkill then
                        -- Recompute with this activity as "active"
                        let
                            fakeGame =
                                { game | activity = Just ( activity, Timer.create ) }

                            mods =
                                Game.getAllEffectMods fakeGame
                            ...
                            duration : Duration
                            duration =
                                Game.getModdedDuration fakeGame activity
                        in
                        { effects = moddedEffects, duration = duration }
        ...
        _ ->
            -- Not viewing a skill tab, just use gameplay cache
            gameplayCache
